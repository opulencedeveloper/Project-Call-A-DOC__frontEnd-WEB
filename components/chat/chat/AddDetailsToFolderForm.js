...
const AddDetailsToFolderForm = (props) => {
  ...
  const { addFolderHandler, token, appointmentId, reloadChatFolderInfo } =
    props;
  ...
  const addDetailsHandlerToFolderSuccessHandler = () => {
  ...
  const addDetailstoPatientRequestResponse = (res) => {
    ...
    if (status === "success") {
      ...
      addDetailsHandlerToFolderSuccessHandler();
    reloadChatFolderInfo();
    }
  };
  ...
  return (
    <BackDrop>
      {isSuccess ? (
        <SuccessMessage
        successMessageHandler={addFolderHandler}
        ...
        >
          ...
          {isLoading ? (
           <div className="flex flex-col items-center justify-center h-full w-full"> <LoadingSpinner /></div>
          ) : (
            <>
